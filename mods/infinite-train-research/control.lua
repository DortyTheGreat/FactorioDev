...
function mod.tick_cargo_wagon(cargo_wagon)
  ...
  -- 0 for no research
  local upgrade_level_effect = game.get_player(1).force.technologies["cargo-wagon-capacity"].level - 1
  ...
  local next_inventory_size = default_inventory_size * (upgrade_level_effect / 2 + 1)
  
  -- sadly, I can't change the weight... cargo_wagon.weight = (1000 + upgrade_level_effect * 500)
...
function mod.on_update_event(event)
...
for _, event in ipairs({
  ...
  script.on_event(event, mod.on_update_event)
end